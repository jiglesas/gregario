    ...
    for wp_s in sublocal:
        ...
        for wp_a in result_aisles:
            ...
            for item in range(0,items):
                ...
                name = data[item]['name']
                id_ = data[item]['id']
                offer_price = str(data[item]['price'])
                ...
                #print(aisle_id)
                info = {'store_id': store_id['store_id'], 'aisle_id': aisle_id['aisle_id'], 'name': str(name), 'id':id_, 'offer_price': str(offer_price),
                        'price_wo_offer': str(price_wo_offer), 'price_per_unit': str(price_per_unit), 'package': str(package),
                        'status': str(status), 'brand': str(brand), 'offer': str(offer), 'create_date': str(date)}
...
df.head(0).to_sql(config.CORNERSHOP_PRODUCT_DETAIL_TEST['table_lz'], engine, if_exists='append',index=False)
...
output.seek(0)
cur.copy_from(output, config.CORNERSHOP_PRODUCT_DETAIL_TEST['table_lz'], null="") # null values become ''
conn.commit()
