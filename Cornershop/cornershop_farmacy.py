...
result_stores = connection.execute(config.CORNERSHOP_STORES['query_farmacy']).fetchall()
result_aisles = connection.execute(config.CORNERSHOP_AISLES['query_farmacy']).fetchall()
...
df.head(0).to_sql(config.CORNERSHOP_PRODUCT_DETAIL['table_farmacy'], engine, if_exists='append',index=False)
...
output.seek(0)
cur.copy_from(output, config.CORNERSHOP_PRODUCT_DETAIL['table_farmacy'], null="") # null values become ''
conn.commit()
