...
def stores_aisles(inicio,fin):
    ...
    driver = webdriver.Chrome('H:/scrapingCornershop/chromedriver.exe', options=options)
    local = []
...
result_stores = connection.execute(config.CORNERSHOP_STORES_MX['query_stores']).fetchall()
result_aisles = connection.execute(config.CORNERSHOP_AISLES_MX['query_aisles']).fetchall()
...
df.head(0).to_sql(config.CORNERSHOP_PRODUCT_MX['table_lz'], engine, if_exists='append',index=False)
...
output.seek(0)
cur.copy_from(output, config.CORNERSHOP_PRODUCT_MX['table_lz'], null="") # null values become ''
conn.commit()
