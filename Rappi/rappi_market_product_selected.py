...
result_stores = connection.execute(config.RAPPI_MARKET_PRODUCT['query_market']).fetchall()
stores_len = len(result_stores)

#version no productiva
...
#    get_product(int(store_in['store_id']), str(result_stores[i]['category_id']), 0)
